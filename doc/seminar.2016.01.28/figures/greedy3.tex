...
\path<3-> node[myrectang, rounded corners, \cola] at (7.03 * \len, 0.6cm) {$a(P_1) = 2n~(\times 1)$};
...
\path<4-> node[myrectang, rounded corners, \colb] at (7 * \len, 0) {$a(P_2) = 1~(\times n)$};
...
\path<5-> node[myrectang, rounded corners, \colc] at (7 * \len, -0.6cm) {$a(P_3) = 1~(\times n)$};
