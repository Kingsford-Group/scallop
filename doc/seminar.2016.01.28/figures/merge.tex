...
% G1
\begin{scope}[local bounding box=bbox, xshift=-6.6cm, yscale=1.2]
...
\path<2-> node[myrectang, rounded corners, \colc] at (7.03 * \len, 0.0cm) {$a(P_1) = a(P_2)$};
\path<3-> [draw, line width = 0.08cm, <->] (3.5 * \len, -0.4cm) to node[label=right:{$P_1 + P_2 = P_1' + P_2'$}]{} (3.5 * \len, -1.5cm);
\end{scope}

% G2
\begin{scope}[local bounding box=bbox, xshift=-6.6cm, yscale=1.2, yshift = -2.0cm]
...
\path<3-> node[myrectang, rounded corners, \colc] at (7.03 * \len, 0.0cm) {$a(P_1') = a(P_2')$};
\end{scope}
