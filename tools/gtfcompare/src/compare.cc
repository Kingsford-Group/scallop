	...
	for(int i = 0; i < x.size(); i++)
	{
		...
		for(int j = 0; j < y.size(); j++)
		{
			...
			if(mode == 2)
			{
				...
				if(b == true)
				{
					printf("TRUE %s %lu %s %s %c %d %lu %d %s %lu %s %s %c %d %lu %d\n", 
							t1.gene_id.c_str(), x.size(), t1.transcript_id.c_str(), t1.label().c_str(), t1.strand, t1.expression, t1.exons.size(), t1.length(),
							t2.gene_id.c_str(), y.size(), t2.transcript_id.c_str(), t2.label().c_str(), t2.strand, t2.expression, t2.exons.size(), t2.length());
				}
		...
		if(flag == false) printf("FALSE %s %lu %s %s %c %d %lu %d\n", 
				t1.gene_id.c_str(), x.size(), t1.transcript_id.c_str(), t1.label().c_str(), t1.strand, t1.expression, t1.exons.size(), t1.length());
	}
	return cnt;
}
