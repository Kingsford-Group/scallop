...
barx = barplot(t(x[,q]), beside=TRUE, col=cc, names.arg=t(x[,1]), ylim=c(0,maxy), ylab = "Sensitivity (%)")
axis(2, seq(0,maxy,by=10))

legend(5.0, maxy, c("Scallop(TopHat, STAR)", "StringTie(TopHat, STAR)", "TransComb(TopHat, STAR)"), fill=c(2,3,4));
dev.off()
...
barx = barplot(t(x[,q]), beside=TRUE, col=cc, names.arg=t(x[,1]), ylim=c(0,maxy), ylab = "Precision (%)")
axis(2, seq(0,maxy,by=10))
