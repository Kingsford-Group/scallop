...
scallop::scallop(const config &c)
	: cfg(c), hs(c)
{}

scallop::scallop(const splice_graph &g, const hyper_set &h,const config &c)
	: gr(g), hs(h), cfg(c)
{
	round = 0;
...
{
	int c = classify();
	if(cfg.verbose >= 1) printf("process splice graph %s type = %d, vertices = %lu, edges = %lu\n", gr.gid.c_str(), c, gr.num_vertices(), gr.num_edges());
