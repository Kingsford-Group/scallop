...
optimizer::optimizer(const splice_graph &g, vector<path> &p)
	: gr(g), paths(p)
{
	env = new GRBEnv();
...
int optimizer::solve()
{
	...
	add_path_weight_variables();

	add_vertex_weight_constraints();
...
int optimizer::add_vertex_weight_variables()
{
	vnwt.clear();
...
int optimizer::add_vertex_error_variables()
{
	verr.clear();
	for(int i = 0; i < num_vertices(gr); i++)
	{
...
int optimizer::add_edge_weight_variables()
{
	enwt.clear();
...
int optimizer::add_edge_error_variables()
{
	eerr.clear();
...
int optimizer::add_vertex_error_constraints()
{
	for(int i = 0; i < num_vertices(gr); i++)
	{
...
int optimizer::add_edge_error_constraints()
{
	for(int i = 0; i < num_edges(gr); i++)
	{
...
int optimizer::add_conservation_constraints()
{
