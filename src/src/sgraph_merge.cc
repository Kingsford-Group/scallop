...
#include "sgraph_merge.h"
#include "util.h"
#include "draw.h"

sgraph_merge::sgraph_merge(const splice_graph &g1, const splice_graph &g2, const hyper_set &h1, const hyper_set &h2)
	:gr1(g1), gr2(g2), hs1(h1), hs2(h2)
{}

int sgraph_merge::compare(const string &file)
{
	imap.clear();
...
int sgraph_merge::build_split_interval_map(splice_graph &gr)
{
...
int sgraph_merge::add_vertices(splice_graph &gr)
{
...
int sgraph_merge::add_inner_edges(splice_graph &gt, splice_graph &gr, int type)
{
...
int sgraph_merge::add_existing_edges(splice_graph &gt, splice_graph &gr, int type)
{
...
int sgraph_merge::search_splice_graph(splice_graph &gr, int32_t p)
{
...
int sgraph_merge::compare_boundary_edges()
{
	int tp = 0, fp = 0, fn = 0;
...
int sgraph_merge::compare_splice_positions()
{
	int tp = 0, fp = 0, fn = 0;
...
bool sgraph_merge::verify_unique_5end_edge(splice_graph &gr, edge_descriptor e)
{
	int t = e->target();
...
bool sgraph_merge::verify_unique_3end_edge(splice_graph &gr, edge_descriptor e)
{
	int s = e->source();
	int type = gr.get_edge_info(e).type;
...
int sgraph_merge::draw(splice_graph &gr, const string &file)
{
